...
*                                                                      *
*======================================================================*
* Programa...: ZR_UPLOAD_REQUEST                                       *
* Módulo.....:                                                         *
* Propósito..: Faz o upload de uma request local para o SAP            *
*----------------------------------------------------------------------*
...
*            |          |                |                             *
*======================================================================*
REPORT zr_upload_request.

TYPE-POOLS: abap, sabc, stms.

CONSTANTS: gc_tp_fillclient LIKE stpa-command VALUE 'FILLCLIENT'.

CONSTANTs: c_8192 type p value 8192.

DATA: g_file_size_max_line type p.
...
INITIALIZATION.
  bl_title = 'Parâmetros de importação'(b01).
  comm_sel = 'Selecionar a co-file. O nome deve iniciar com ''K''.'(001).
...
AT SELECTION-SCREEN.
  ...
  IF sl < 11.
    MESSAGE e001(00)
    WITH
'Invalid co-file name format. File name format must be KNNNNNNN.' &
'SSS'(009)
    .
  ENDIF.
  sl = sl - 11.
...
START-OF-SELECTION.
  ...
  g_file_size_max_line = c_8192 * cl_abap_char_utilities=>charsize.
...
FORM copy_file USING subdir fname source_file.
  ...
  ENDIF.
  LOOP AT datatab.
    "Estava fixo 8192, mas em unicode é 16384
    IF flen < g_file_size_max_line.
      len = flen.
    ELSE.
      len = g_file_size_max_line.
    ENDIF.
